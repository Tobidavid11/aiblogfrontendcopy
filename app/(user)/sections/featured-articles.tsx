...
const FeaturedArticles = () => {
  ...
  useEffect(() => {
    const fetchArticles = async () => {
      try {
        const response = await fetch("https://gateway.drello.xyz/api/v1/blog?featured=true"); 
        ...
        const parsedArticles = data.data.results.map((item: any): ArticleProps => ({
          ...
          description: item.content.replace(/<\/?[^>]+(>|$)/g, "").slice(0, 100), // Basic sanitization and truncation
