...
const UserInfo: React.FC<ProfileCardProps> = ({ user, className , isFolloweBy, isExternal }) => {
  ...
  return (
    <div className={cn("flex flex-col  justify-between gap-2 px-4", className)}>
      <div className="md:w-[70%] flex gap-2 mb-3 items-center">
        <div className="flex-1 gap-y-1">
          ...
          <p className="text-xs font-normal mb-3 mt-1 pb-2 flex items-center gap-2">
            @{user?.username} {isFolloweBy && <span className=" text-[10px] p-[4px] leading-tight rounded-sm">follows you</span>}
          </p>
      ...
      <div className="flex justify-start  gap-4 my-3">
        <Link href={!isExternal ? "/follow":`/profile/${user.username}/follow`} className="text-xs font-bold flex items-center gap-2  ">
